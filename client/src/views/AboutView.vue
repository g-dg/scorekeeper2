...
async function loadLicense() {
  license.value = await (await fetch(`${API_URI}about/license`, { mode: "cors", redirect: "follow" })).text();
}
onMounted(loadLicense);

const serverVersion = ref<string>();
async function loadVersion() {
  serverVersion.value = await (await fetch(`${API_URI}about/version`, { mode: "cors", redirect: "follow" })).text();
}
